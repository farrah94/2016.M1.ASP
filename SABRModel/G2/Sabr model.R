...
CalcSabrPriceMC <- function(forward, spot=forward*exp(-pr*t.exp),strike,sigma0,alpha,
    ...
    #Return: the option price
    dt = t.exp/n.periods
    s = vol =matrix(0,n.periods,n.sample)
    s[1,] = spot*exp(r*t.exp)
    vol[1,] = sigma0
    call=vector(length=n.sample)
    payoffs <- vector(length=n.sample)
...
CalcSabrPriceKennedy <- function(forward,spot=forward*exp(-pr*t.exp),strike,sigma0,alpha,beta=0,rho,t.exp,r,nodes=50){
    #------------------------------------------------
    #-----------only support beta=0 so far-----------
    #Imput: spot is current price
    #       strike is the strike price vector (length is 3)
